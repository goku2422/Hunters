...
import { Gift, Home, MapPin, QrCode, ScanLine, UserRound, Zap, ChevronRight, LogOut, Phone, Store, Camera, AlertCircle, Search, Navigation } from 'lucide-react';
...
function HomeHeader({ customerName }: { customerName: string }) {
    return <section className="rounded-b-[28px] bg-gradient-to-br from-[#a80713] to-[#c71924] px-5 pb-7 pt-7 text-white shadow-lg shadow-red-900/10"><div className="flex items-center gap-3"><div className="flex h-11 w-11 items-center justify-center rounded-xl border border-white/25 bg-white/10 text-xs font-black">Kutio</div><div><p className="text-[10px] font-bold uppercase tracking-wider text-white/75">Good afternoon,</p><h1 className="text-base font-black">{customerName}</h1></div></div><h2 className="mt-6 text-lg font-black">Eight visits to your first reward.</h2><p className="mt-1 text-sm text-white/80">Start a card below, or scan at the counter.</p></section>;
}
...
function ExploreView({ shops, loading }: { shops: CustomerShop[]; loading: boolean }) {
    return <div><h2 className="text-base font-bold">Explore Stores</h2><p className="mt-1 text-sm text-[#8390a2]">Discover places where you can earn rewards.</p><div className="mt-5 space-y-3">{loading ? <p className="text-sm text-[#8390a2]">Loading stores...</p> : shops.map((shop) => <div key={shop.id} className="rounded-2xl bg-white p-4 shadow-sm ring-1 ring-slate-100"><div className="flex items-start justify-between gap-3"><div><h3 className="font-bold">{shop.name}</h3><p className="mt-1 text-xs text-[#8390a2]">{shop.category}</p><p className="mt-2 flex items-center gap-1 text-xs text-[#667085]"><MapPin className="h-3.5 w-3.5 text-[#b20d18]" />{shop.address}</p></div><Store className="h-5 w-5 text-[#b20d18]" /></div><Link href="/claim" className="mt-4 flex items-center justify-center rounded-xl bg-[#fff0f0] py-2.5 text-xs font-bold text-[#b20d18]">Scan at this store <ChevronRight className="ml-1 h-4 w-4" /></Link></div>)}</div></div>;
}
...
function StampCardView({ shop, onBackHome }: { shop: CustomerShop; onBackHome: () => void }) {
    ...
    return <div className="-mx-5 -mt-6 min-h-[620px] bg-[#f6f7f8] pb-10"><section className="rounded-b-[28px] bg-gradient-to-br from-[#a80713] to-[#c71924] px-5 pb-7 pt-7 text-white shadow-lg shadow-red-900/10"><button type="button" onClick={onBackHome} className="text-xs font-bold text-white/80">← Back</button><div className="mt-5 flex items-center gap-3"><div className="flex h-12 w-12 items-center justify-center rounded-full bg-white text-lg font-black text-[#b20d18]">{shop.name.charAt(0)}</div><h1 className="text-lg font-black">{shop.name}</h1></div><p className="mt-8 text-3xl font-black">0 of 8 Stamps</p><div className="mt-7 h-2 rounded-full bg-white/25"><div className="h-full w-0 rounded-full bg-white" /></div></section><div className="mx-4 -mt-4 grid grid-cols-2 overflow-hidden rounded-full bg-white shadow-sm"><button type="button" className="rounded-full bg-[#a80713] py-3 text-sm font-bold text-white"><Gift className="mr-2 inline h-4 w-4" /> Rewards</button><button type="button" className="py-3 text-sm font-semibold text-slate-500">🍴 Menu</button></div><section className="mx-4 mt-6 flex items-center gap-3 rounded-2xl bg-white p-4 shadow-sm"><div className="flex h-14 w-14 items-center justify-center rounded-xl bg-[#f8e6e6] text-2xl">🎁</div><div className="flex-1"><h2 className="text-sm font-bold">Get 5% discount on your total bill after 8 visits</h2><p className="mt-1 text-[10px] font-bold text-[#b20d18]">8 STAMPS <span className="font-normal text-slate-400">• Collect 8 more</span></p></div><span className="text-[10px] font-bold text-slate-700">30 DAY Expiry</span></section><section className="mx-4 mt-7 rounded-2xl bg-white p-5 shadow-sm"><p className="text-[10px] font-bold uppercase tracking-wider text-slate-400">Stamp Card</p><div className="mt-5 flex justify-between gap-1">{Array.from({ length: 8 }, (_, index) => <div key={index} className={`flex h-14 w-14 items-center justify-center rounded-2xl border-2 border-dashed text-lg font-bold ${index === 7 ? 'border-[#ff7777] bg-[#fff0f0] text-[#ff7777]' : 'border-[#d7dee8] text-[#e2e6ec]'}`}>{index === 7 ? <Gift className="h-5 w-5" /> : index + 1}</div>)}</div><p className="mt-6 text-center text-sm text-slate-500">You&apos;re <span className="font-bold text-[#b20d18]">8 stamps</span> away from <span className="font-bold text-[#b20d18]">5% discount on your total bill after 8 visits</span></p></section><section className="mx-4 mt-7"><p className="text-[10px] font-bold uppercase tracking-wider text-slate-400">Business Info</p><div className="mt-3 rounded-2xl bg-white p-4 text-sm"><p className="font-bold">{shop.name}</p><p className="mt-1 text-xs text-slate-500">{shop.category}</p><p className="mt-3 flex items-center gap-2 text-xs text-slate-600"><MapPin className="h-4 w-4 text-[#b20d18]" /> {shop.address}</p></div></section></div>;
}
...
function Stat({ icon, value, label }: { icon: ReactNode; value: string; label: string }) { return <div className="rounded-xl border border-white/20 bg-white/10 px-2 py-2.5 text-center"><div className="flex justify-center text-white/90">{icon}</div><div className="mt-0.5 text-lg font-black leading-5">{value}</div><div className="text-[8px] font-bold uppercase tracking-wider text-white/75">{label}</div></div>; }
